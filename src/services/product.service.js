...
const getProductDetailForCustomer = async (productId) => {
    ...
    const { productImages, isDeleted, categoryId, category, ...basicInfo } = productDetail.get({ plain: true });;
    const priceDiscounted = basicInfo.discountOffers[0]?.priceDiscounted || basicInfo.price;
